...
docker exec samba-dc samba-tool group add "All_Company" --mail-address="all@wigitron.com"
docker exec samba-dc samba-tool group add "Support_Team" --mail-address="support@wigitron.com"
docker exec samba-dc samba-tool group add "Marketing_Team" --mail-address="marketing@wigitron.com"

# Add all 20 existing test users to the 'All_Company' group
echo "Adding all users to All_Company (all@wigitron.com)..."
for i in {1..20}; do
  docker exec samba-dc samba-tool group addmembers "All_Company" "testuser${i}"
done

# Assign a subset of users to the Support group
echo "Assigning users 1-5 to Support_Team (support@wigitron.com)..."
for i in {1..5}; do
  docker exec samba-dc samba-tool group addmembers "Support_Team" "testuser${i}"
done

# Assign a different subset to the Marketing group
echo "Assigning users 6-10 to Marketing_Team (marketing@wigitron.com)..."
for i in {6..10}; do
